...
function createCard(data, type) {
  if (type === 'hospital') {
    const imageUrl = data.image ? `http://localhost:3000${data.image}` : 'default-hospital.jpg';
    return `
...
hospitalSearch?.addEventListener('input', async () => {
  ...
  try {
    const res = await fetch(`http://localhost:3000/api/hospitals?query=${encodeURIComponent(query)}`);
...
bankSearch?.addEventListener('input', async () => {
  ...
  try {
    const res = await fetch(`http://localhost:3000/api/bloodbanks?query=${encodeURIComponent(query)}`);
